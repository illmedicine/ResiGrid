...
const COMPETITORS = [
  {
    name: "DoorLoop",
    color: "#6C3FF2",
    tiers: ["Starter $69/mo", "Pro $109/mo", "Premium $209/mo"],
    tenantFee: "ACH fees unless Premium tier ($209/mo)",
    billing: "Monthly flat rate per unit block",
    gated: true,
    vacant: true,
  },
  {
    name: "AppFolio",
    color: "#0070F3",
    tiers: ["$280–$298/mo minimum"],
    tenantFee: "$2.49 per ACH payment",
    billing: "$1.40/unit/month (must hit minimum)",
    gated: false,
    vacant: true,
  },
];
...
export default function PricingPage() {
  return (
    <div className="flex flex-1 flex-col bg-neutral-50">
      ...
      {/* ── Tenant Promise Banner ─────────────────────────────────── */}
      <div className="bg-navy-900 px-4 py-3 text-center">
        <p className="text-sm font-semibold text-white">
          <span className="text-orange-400">The Tenant Promise</span>
          {" · "}
          Cost to pay rent online:{" "}
          <span className="text-orange-300">$0.00 — forever.</span>{" "}
          <span className="text-white/60 text-xs">
            Standard ACH transfers are always free. No exceptions.
          </span>
        </p>
      </div>
      ...
      <section className="bg-navy-900 px-4 pb-16 pt-12 text-center">
        ...
        <h1 className="mx-auto max-w-2xl text-3xl font-bold text-white md:text-4xl">
          ...
          <span className="text-orange-400">Never for Empty Ones.</span>
        </h1>
        <p className="mx-auto mt-4 max-w-xl text-sm text-white/70 md:text-base">
          A low annual onboarding fee to join the Grid, then exactly{" "}
          <strong className="text-orange-300">$1 per month</strong> for every
          occupied unit. When a unit is vacant, you don&apos;t pay for it.
          All premium features included in every tier — nothing gated.
        </p>
      </section>
            ...
            >
              ...
              <p
                className={`mt-1 text-sm ${
                  highlighted ? "text-white/70" : "text-neutral-500"
                }`}
              >
                {tier.tagline}
              </p>
              ...
              <div className="mt-5">
                ...
                <span
                  className={`ml-1 text-sm ${
                    highlighted ? "text-white/60" : "text-neutral-500"
                  }`}
                >
                  / year onboarding
                </span>
              ...
              <ul className="mt-5 flex flex-col gap-2 text-xs">
                ...
                />
                <FeatureItem
                  text="$0 tenant ACH fee — always"
                  highlighted={highlighted}
                />
                <FeatureItem
                  text="No charge for vacant units"
                  highlighted={highlighted}
                />
                <FeatureItem
                  text="No feature gating"
                  highlighted={highlighted}
                />
              </ul>
      ...
      <section className="bg-white px-4 py-16">
        ...
        <p className="mx-auto mb-10 max-w-xl text-center text-sm text-neutral-600">
          Unlike DoorLoop — which locks Free Tenant ACH and custom websites behind
          their $209/month Premium tier — ResiGrid gives you our entire feature
          suite from day one, on every plan.
        </p>
        <div className="mx-auto grid w-full max-w-4xl gap-4 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5">
          {FEATURES.map(({ icon: Icon, label }) => (
            <div
              key={label}
              className="flex flex-col items-center gap-2 rounded-xl border border-neutral-100 bg-neutral-50 p-4 text-center"
            >
        ...
        <div className="mx-auto max-w-4xl overflow-x-auto">
          <table className="w-full text-sm">
            <thead>
              <tr>
                ...
                {COMPETITORS.map((c) => (
                  <th
                    key={c.name}
                    className="pb-4 pr-4 text-center text-xs font-semibold uppercase tracking-wider text-neutral-500"
                  >
                    {c.name}
                  </th>
                ))}
                <th className="pb-4 text-center text-xs font-semibold uppercase tracking-wider text-orange-500">
                  ResiGrid
                </th>
              </tr>
            </thead>
            <tbody className="divide-y divide-neutral-200">
              <CompareRow
                label="Entry price"
                doorloop="$69/month (≤20 units)"
                appfolio="$280–$298/month minimum"
                resigrid="$40/year + $1/mo per occupied unit"
                resigridWins
              />
              <CompareRow
                label="Tenant ACH fee"
                doorloop="Charged unless $209/mo Premium"
                appfolio="$2.49 per payment"
                resigrid="$0.00 — always free"
                resigridWins
              />
              <CompareRow
                label="Pay for vacant units?"
                doorloop="Yes — billed regardless"
                appfolio="Yes — minimum applies"
                resigrid="Never — only occupied units billed"
                resigridWins
              />
              <CompareRow
                label="Feature gating"
                doorloop="Yes — best features need $109–$209/mo"
                appfolio="Complex tiered unlocks"
                resigrid="None — all features on every tier"
                resigridWins
              />
              <CompareRow
                label="RGE reputation network"
                doorloop="—"
                appfolio="—"
                resigrid="Included on every tier"
                resigridWins
              />
            </tbody>
          </table>
      ...
      <section className="bg-navy-900 px-4 py-16 text-center">
        ...
        <p className="mx-auto mt-4 max-w-xl text-sm text-white/70">
          ...
          revenue. Empty units cost you nothing. Tenant payment fees cost your
          renters nothing. Scale with occupancy, not unit count.
        </p>
        <Link
          href="/signup?role=property_manager"
      ...
      <td className="py-3.5 text-center">
        <span
          className={`inline-flex items-center gap-1 text-xs font-semibold ${
            resigridWins ? "text-green-600" : "text-neutral-600"
          }`}
        >
